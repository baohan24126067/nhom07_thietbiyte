...
export function ProductDetailTabs({ items }: { items: ProductDetailTab[] }) {
  ...
  return (
    <section className="mt-6 grid gap-5 lg:grid-cols-[0.35fr_0.65fr]">
      <aside className="rounded-[8px] border border-[var(--color-line)] bg-[var(--color-card)] p-5 shadow-[0_20px_60px_rgba(17,57,95,0.08)]">
              ...
              <button
                ...
                className={`w-full rounded-[8px] border px-4 py-3 text-left text-sm font-semibold transition ${
                  isActive
                    ...
                    : "border-[var(--color-line)] bg-[var(--color-card)] text-[var(--color-ink)] hover:border-[var(--color-brand)] hover:bg-[var(--color-brand-soft)]"
                }`}
              >
                {item.title}
              </button>
      ...
      <article className="rounded-[8px] border border-[var(--color-line)] bg-[var(--color-card)] p-5 shadow-[0_20px_60px_rgba(17,57,95,0.08)]">
        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--color-brand)]">
          {activeItem.title}
        </p>
